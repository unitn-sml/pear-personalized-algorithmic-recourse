...
from competitors.watcher.utils import wachter_recourse
...
class Wachter(RecourseMethod):
    ...
    def get_counterfactuals(self, factuals: pd.DataFrame, W, G=None) -> pd.DataFrame:
        
        cat_features_indices = {k:[] for k in self._mlmodel.data._preprocessor.categorical}

        encoded_feature_names = self._mlmodel.data._preprocessor.categorical_encoded
        for feature_name in cat_features_indices.keys():
            for feature_name_encoded in encoded_feature_names:
                if feature_name_encoded not in self._mlmodel.data._preprocessor.continuous:
                    if feature_name in feature_name_encoded:
                        cat_features_indices[feature_name].append(
                            factuals.columns.get_loc(feature_name_encoded)
                        )

        df_cfs = []
        for idx, x in tqdm(factuals.iterrows()):
            ...
            df_cfs.append(
                wachter_recourse(
                    ...
                    x.values,
                    W.iloc[[idx]],
                    G.iloc[[idx]] if G else None,
                    factuals.columns,
                    y_target=self._y_target,
                    immutables=self._mlmodel.data.immutables,
        ...
        df_cfs = pd.DataFrame(df_cfs, columns=factuals.columns)
