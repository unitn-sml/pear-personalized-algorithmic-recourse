...
from competitors.cscf.decoder import Decoder
from competitors.cscf.utils.gowers import gower_matrix
...
class ProblemFactory(Problem):
    ...
    def __init__(
        ...
        self.init_observation = self.env.features.copy()
        self.x0 = self.env.get_observation() # Environment
    ...
    def get_costs(self, sequences):
        ...
        def parallelize(x, env, init_observation):
            ...
            if x is None:
                ...
                env.features = init_observation.copy()

                assert not env.prog_to_postcondition(None, env.features.copy())
                ...
                for p, a in x:
                    ...
                    if not precondition_satisfied:
                        cost_single.append(10000)
                        continue

                    pindex = env.programs_library[p].get("index")
                    aindex = env.inverse_complete_arguments.get(a).get(p)

                    # check preconditions
                    if aindex is None:
                        precondition_satisfied = False
                    elif not env.prog_to_precondition.get(p)(a):
                        precondition_satisfied = False

                    if precondition_satisfied:
                        cost_single.append(env.get_cost(pindex, aindex))
                        env.prog_to_func[p](a)
                    else:
                        cost_single.append(10000)

                evaluation_result = env.prog_to_postcondition(None, env.features.copy())
                ...
                if not precondition_satisfied:
                    ...
                    evaluation_result = evaluation_result and not (env.features.copy()==init_observation.copy()) and len(x) > 1
